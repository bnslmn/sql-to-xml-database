...
-- Type ContCountries
DROP TYPE TcontCountries FORCE;
/
CREATE OR REPLACE TYPE T_contCountries AS OBJECT(
    contCode VARCHAR2(20),
...
output XMLType;
    ...
    begin
        output :=  XMLType.createxml('<country name = "'||nameC||'" continent = "'||continentC||'"/>');
...
SELECT T_Country(c.code , c.name , e.continent)
FROM Country c, ENCOMPASSES e 
WHERE c.code = e.country;

INSERT INTO ex2 VALUES ('exo2_3');

-- exporter le r�sultat dans un fichier 
WbExport -type=text
         -file='testcountries.xml'
         -createDir=true
         -encoding=ISO-8859-1
         -header=false
         -delimiter=','
